 ...
 * @returns
 */
export default function(scene, levelX, levelY, transition = true, transitionPosition = null) {
    return new Promise((resolve, reject) => {
        let level = null;
        let previousLevel = scene.currentLevel;
        try {
            import(`./Level${levelX}_${levelY}`).then(async (LevelClass) => {
                ...
                level.preload(true).then(() => {
                    ...
                    if (transition && !transitionPosition && previousLevel) {
                        transitionPosition = determineTransitionPosition(scene.igor, previousLevel, level);
                    }
...
function buildLevel(scene, level) {
    ...
    level.map.forEach((el, i) => {
        ...
        let y = (level.tileHeight*Math.floor(i/level.width)) + level.tileHeight/2;
...
function determineTransitionPosition(player, levelFrom, levelTo) {
    ...
    if (levelFrom.x < levelTo.x) {
        ...
        position.y = levelTo.height * levelTo.tileHeight - (levelTo.tileHeight*2);
    }
    else if (levelFrom.y > levelTo.y) {
