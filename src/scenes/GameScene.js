...
class GameScene extends Phaser.Scene {
    ...
    initLevel = () => {
        ...
        if (this.currentLevel) {
            this.events.emit('level.destroy')
        ...
        this.middleGroup.add(wallsLayer)
        ...
        map.getObjectLayer('Objects').objects.forEach((obj) => {
            ...
            console.log(obj)
            let TileClass = Tiles[obj.name]
            let block = new TileClass({ scene: this, x, y, tileset: 'base' })
            if (block.onOverlap) {
                this.physics.add.overlap(
                    this.igor,
                    block.sprite,
                    (igor, blockSprite) => {
                        let block = blockSprite.getData('tile')
                        block.onOverlap(igor)
                    }
                )
            }
        ...
        this.middleGroup.setDepth(10000)
        this.foregroundGroup.setDepth(20000)
        ...
        if (this.igor.y < this.igor.height / 2) {
            this.loadLevel(this.levelCoords.x, ++this.levelCoords.y).then(
                () => {
                    this.initLevel()
                    this.igor.y =
                        this.currentLevel.heightInPixels - this.igor.height
                }
            )
            ...
            this.loadLevel(--this.levelCoords.x, this.levelCoords.y).then(
                () => {
                    this.initLevel()
                    this.igor.x =
                        this.currentLevel.widthInPixels - this.igor.width
                }
            )
